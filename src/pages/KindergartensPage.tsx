...
import kindergartensData from '../data/kindergartens.json';

interface KindergartensPageProps {
  onBack: () => void;
  onNavigate?: (page: any) => void;
}

export const KindergartensPage: React.FC<KindergartensPageProps> = ({ onBack, onNavigate }) => {
  useEffect(() => {
    window.scrollTo(0, 0);
  }, []);
  ...
  const regions = Object.keys(kindergartensData).sort((a, b) => {
  ...
  const items = useMemo(() => {
    // @ts-ignore
    return (kindergartensData[selectedRegion] || []).slice(0, 60);
  }, [selectedRegion]);
    ...
    <div className="min-h-screen bg-black text-white selection:bg-white selection:text-black pb-20">
      ...
      <section className="pt-40 pb-12 px-6 max-w-7xl mx-auto flex flex-col items-center text-center">
        <motion.p
          ...
          className="text-yellow-400 text-[14px] tracking-[0.3em] uppercase font-bold mb-4"
        >
        ...
        >
          낮 시간 케어, 사회성 훈련 및 체계적인 유치원 프로그램을 제공하는 전국 인허가 애견유치원 정보입니다.<br/>
          <span className="text-sm text-white/50 mt-2 block tracking-tight">※ 본 데이터는 2025년 11월 28일 공공데이터 기준이며, 방문 전 유선 확인을 권장합니다.</span>
        </motion.p>
      </section>
          ...
          {items.map((facility: any, index: number) => {
            ...
            return (
              <motion.a
                ...
                className="group flex items-center gap-4 bg-white/5 border border-white/10 rounded-full py-3 px-5 hover:bg-white/10 hover:border-yellow-400/50 transition-colors cursor-pointer w-full"
              >
                <div className="w-8 h-8 rounded-full flex-shrink-0 bg-white/10 flex items-center justify-center font-bold text-sm text-white/80 group-hover:bg-yellow-400 group-hover:text-black transition-colors">
                  {index + 1}
                </div>
                <div className="flex flex-1 items-center gap-4 overflow-hidden">
                  <h3 className="text-base font-bold whitespace-nowrap text-white/90 group-hover:text-yellow-400 flex-shrink-0">
                    {facility.name}
                  </h3>
