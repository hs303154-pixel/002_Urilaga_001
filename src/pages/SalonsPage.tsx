...
interface SalonsPageProps {
  onBack: () => void;
  onNavigate?: (page: 'home' | 'party' | 'fashion' | 'food' | 'salon') => void;
}

export const SalonsPage: React.FC<SalonsPageProps> = ({ onBack, onNavigate }) => {
  useEffect(() => {
    window.scrollTo(0, 0);
  }, []);
  ...
  const regions = Object.keys(salonsData).sort((a, b) => {
  ...
  const salons = useMemo(() => {
    // @ts-ignore
    return (salonsData[selectedRegion] || []).slice(0, 50); // Show top 50 for performance
  }, [selectedRegion]);
    ...
    <div className="min-h-screen bg-black text-white selection:bg-white selection:text-black pb-20">
      ...
      <nav className="fixed top-0 left-0 right-0 z-50 h-20 px-6 sm:px-8 flex items-center justify-between backdrop-blur-md bg-black/50 border-b border-white/10">
        ...
        <div className="flex items-center gap-3 overflow-x-auto">
          {onNavigate && (
            <>
              <button onClick={() => onNavigate('salon')} className="px-5 py-2.5 bg-fuchsia-500 hover:bg-fuchsia-400 text-white rounded-[14px] text-sm font-bold tracking-wider backdrop-blur-md transition-all active:scale-95 shadow-sm">동물 미용실</button>
      ...
      <section className="pt-40 pb-12 px-6 max-w-7xl mx-auto flex flex-col items-center text-center">
        ...
        >
          우리아가와 언제 어디서든 안심하고 함께할 수 있도록, 전국 동물 미용실 데이터를 제공합니다.<br/>
          <span className="text-sm text-white/50 mt-2 block tracking-tight">※ 본 데이터는 2025년 8월 11일 공공데이터 기준으로 작성되었으며, 실제 영업 정보와 다를 수 있습니다.</span>
        </motion.p>
      </section>
      ...
      <section className="px-6 max-w-7xl mx-auto">
        <div className="flex flex-col gap-3 w-full max-w-4xl mx-auto">
          {salons.map((salon: any, index: number) => {
            ...
            const shortAddress = salon.address ? salon.address.split(' ').slice(0, 2).join(' ') : '';
            const mapQuery = encodeURIComponent(`${salon.name} ${shortAddress}`);
            return (
              <motion.a
                ...
                <div className="flex flex-1 items-center gap-4 overflow-hidden">
                  ...
                  {salon.phone && (
                    <>
                      ...
                      <p className="text-sm text-white/40 hidden md:block">{salon.phone}</p>
                    </>
                  )}
                </div>
