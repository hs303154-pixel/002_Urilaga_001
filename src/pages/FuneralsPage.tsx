...
import funeralsData from '../data/funerals.json';

interface FuneralsPageProps {
  onBack: () => void;
  onNavigate?: (page: 'home' | 'party' | 'fashion' | 'food' | 'funeral') => void;
}

export const FuneralsPage: React.FC<FuneralsPageProps> = ({ onBack, onNavigate }) => {
  useEffect(() => {
    window.scrollTo(0, 0);
  }, []);
  ...
  const regions = Object.keys(funeralsData).sort((a, b) => {
  ...
  const funerals = useMemo(() => {
    // @ts-ignore
    return (funeralsData[selectedRegion] || []).slice(0, 50); // Show top 50 for performance
  }, [selectedRegion]);
    ...
    <div className="min-h-screen bg-black text-white selection:bg-white selection:text-black pb-20">
      ...
      <nav className="fixed top-0 left-0 right-0 z-50 h-20 px-6 sm:px-8 flex items-center justify-between backdrop-blur-md bg-black/50 border-b border-white/10">
        ...
        <div className="flex items-center gap-3 overflow-x-auto">
          {onNavigate && (
            <>
              <button onClick={() => onNavigate('funeral')} className="px-5 py-2.5 bg-zinc-500 hover:bg-zinc-400 text-white rounded-[14px] text-sm font-bold tracking-wider backdrop-blur-md transition-all active:scale-95 shadow-sm">장례식장/추모공원</button>
      ...
      <section className="pt-40 pb-12 px-6 max-w-7xl mx-auto flex flex-col items-center text-center">
        ...
        >
          우리아가와 언제 어디서든 안심하고 함께할 수 있도록, 전국 장례식장/추모공원 데이터를 제공합니다.<br/>
          <span className="text-sm text-white/50 mt-2 block tracking-tight">※ 본 데이터는 2025년 10월 31일 공공데이터 기준으로 작성되었으며, 실제 영업 정보와 다를 수 있습니다.</span>
        </motion.p>
      </section>
      ...
      <section className="px-6 max-w-7xl mx-auto">
        <div className="flex flex-col gap-3 w-full max-w-4xl mx-auto">
          {funerals.map((funeral: any, index: number) => {
            // 주소가 너무 길면 네이버 지도 검색이 실패할 수 있으므로, '병원명 + 시/도 + 구/군' 형태로 검색어 최적화
            const shortAddress = funeral.address ? funeral.address.split(' ').slice(0, 2).join(' ') : '';
            const mapQuery = encodeURIComponent(`${funeral.name} ${shortAddress}`);
            return (
              <motion.a
                ...
                <div className="flex flex-1 items-center gap-4 overflow-hidden">
                  ...
                  {funeral.phone && (
                    <>
                      ...
                      <p className="text-sm text-white/40 hidden md:block">{funeral.phone}</p>
                    </>
                  )}
                </div>
