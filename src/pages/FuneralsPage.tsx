...
export const FuneralsPage: React.FC<FuneralsPageProps> = ({ onBack, onNavigate }) => {
  ...
  const items = useMemo(() => {
    // @ts-ignore
    return (funeralsData[selectedRegion] || []).slice(0, 60);
  }, [selectedRegion]);
    ...
    <div className="min-h-screen bg-black text-white selection:bg-white selection:text-black pb-20">
      ...
      <section className="pt-40 pb-12 px-6 max-w-7xl mx-auto flex flex-col items-center text-center">
        <motion.p
          ...
          className="text-purple-400 text-[14px] tracking-[0.3em] uppercase font-bold mb-4"
        >
        ...
        >
          사랑하는 반려동물과의 아름다운 마지막 배웅을 위한 전국 동물장묘업 등록 장례식장 및 추모공간 정보입니다.
        </motion.p>
      </section>
          ...
          {items.map((facility: any, index: number) => {
            ...
            return (
              <motion.a
                ...
                className="group flex items-center gap-4 bg-white/5 border border-white/10 rounded-full py-3 px-5 hover:bg-white/10 hover:border-purple-400/50 transition-colors cursor-pointer w-full"
              >
                <div className="w-8 h-8 rounded-full flex-shrink-0 bg-white/10 flex items-center justify-center font-bold text-sm text-white/80 group-hover:bg-purple-400 group-hover:text-black transition-colors">
                  {index + 1}
                </div>
                <div className="flex flex-1 items-center gap-4 overflow-hidden">
                  <h3 className="text-base font-bold whitespace-nowrap text-white/90 group-hover:text-purple-400 flex-shrink-0">
                    {facility.name}
                  </h3>
