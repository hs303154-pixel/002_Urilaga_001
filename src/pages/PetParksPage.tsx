...
import petparksData from '../data/petparks.json';

interface PetParksPageProps {
  onBack: () => void;
  onNavigate?: (page: any) => void;
}

export const PetParksPage: React.FC<PetParksPageProps> = ({ onBack, onNavigate }) => {
  useEffect(() => {
    window.scrollTo(0, 0);
  }, []);
  ...
  const regions = Object.keys(petparksData).sort((a, b) => {
  ...
  const items = useMemo(() => {
    // @ts-ignore
    return (petparksData[selectedRegion] || []).slice(0, 60);
  }, [selectedRegion]);
    ...
    <div className="min-h-screen bg-black text-white selection:bg-white selection:text-black pb-20">
      ...
      <section className="pt-40 pb-12 px-6 max-w-7xl mx-auto flex flex-col items-center text-center">
        <motion.p
          ...
          className="text-cyan-400 text-[14px] tracking-[0.3em] uppercase font-bold mb-4"
        >
        ...
        >
          목줄 없이 맘껏 뛰놀 수 있는 오프리시 펫파크, 전용 잔디 운동장, 신나는 수영장 위치 정보를 제공합니다.<br/>
          ...
          {items.map((facility: any, index: number) => {
            ...
            return (
              <motion.a
                ...
                className="group flex items-center gap-4 bg-white/5 border border-white/10 rounded-full py-3 px-5 hover:bg-white/10 hover:border-cyan-400/50 transition-colors cursor-pointer w-full"
              >
                <div className="w-8 h-8 rounded-full flex-shrink-0 bg-white/10 flex items-center justify-center font-bold text-sm text-white/80 group-hover:bg-cyan-400 group-hover:text-black transition-colors">
                  {index + 1}
                </div>
                <div className="flex flex-1 items-center gap-4 overflow-hidden">
                  <h3 className="text-base font-bold whitespace-nowrap text-white/90 group-hover:text-cyan-400 flex-shrink-0">
                    {facility.name}
                  </h3>
