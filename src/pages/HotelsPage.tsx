...
export const HotelsPage: React.FC<HotelsPageProps> = ({ onBack, onNavigate }) => {
  useEffect(() => {
    window.scrollTo(0, 0);
  }, []);
  ...
  const regions = Object.keys(hotelsData).sort((a, b) => {
  ...
  const items = useMemo(() => {
    // @ts-ignore
    return (hotelsData[selectedRegion] || []).slice(0, 60);
  }, [selectedRegion]);
    ...
    <div className="min-h-screen bg-black text-white selection:bg-white selection:text-black pb-20">
      ...
      <section className="pt-40 pb-12 px-6 max-w-7xl mx-auto flex flex-col items-center text-center">
        <motion.p
          ...
          className="text-indigo-400 text-[14px] tracking-[0.3em] uppercase font-bold mb-4"
        >
        ...
        >
          여행이나 일상 중 안심하고 아이를 맡길 수 있는 인허가 전국 동물호텔(동물위탁관리업) 정보를 제공합니다.<br/>
          ...
          {items.map((facility: any, index: number) => {
            ...
            return (
              <motion.a
                ...
                className="group flex items-center gap-4 bg-white/5 border border-white/10 rounded-full py-3 px-5 hover:bg-white/10 hover:border-indigo-400/50 transition-colors cursor-pointer w-full"
              >
                <div className="w-8 h-8 rounded-full flex-shrink-0 bg-white/10 flex items-center justify-center font-bold text-sm text-white/80 group-hover:bg-indigo-400 group-hover:text-black transition-colors">
                  {index + 1}
                </div>
                <div className="flex flex-1 items-center gap-4 overflow-hidden">
                  <h3 className="text-base font-bold whitespace-nowrap text-white/90 group-hover:text-indigo-400 flex-shrink-0">
                    {facility.name}
                  </h3>
