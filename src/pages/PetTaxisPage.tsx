...
import pettaxisData from '../data/pettaxis.json';

interface PetTaxisPageProps {
  onBack: () => void;
  onNavigate?: (page: any) => void;
}

export const PetTaxisPage: React.FC<PetTaxisPageProps> = ({ onBack, onNavigate }) => {
  useEffect(() => {
    window.scrollTo(0, 0);
  }, []);
  ...
  const regions = Object.keys(pettaxisData).sort((a, b) => {
  ...
  const items = useMemo(() => {
    // @ts-ignore
    return (pettaxisData[selectedRegion] || []).slice(0, 60);
  }, [selectedRegion]);
    ...
    <div className="min-h-screen bg-black text-white selection:bg-white selection:text-black pb-20">
      ...
      <section className="pt-40 pb-12 px-6 max-w-7xl mx-auto flex flex-col items-center text-center">
        <motion.p
          ...
          className="text-orange-400 text-[14px] tracking-[0.3em] uppercase font-bold mb-4"
        >
        ...
        >
          대형견, 병원 방문, 장거리 이동 시 안심하고 이용할 수 있는 전국 인허가 펫택시(동물운송업) 정보입니다.<br/>
          <span className="text-sm text-white/50 mt-2 block tracking-tight">※ 본 데이터는 2026년 4월 16일 공공데이터 기준이며, 사전 유선 확인을 권장합니다.</span>
        </motion.p>
      </section>
          ...
          {items.map((facility: any, index: number) => {
            ...
            return (
              <motion.a
                ...
                className="group flex items-center gap-4 bg-white/5 border border-white/10 rounded-full py-3 px-5 hover:bg-white/10 hover:border-orange-400/50 transition-colors cursor-pointer w-full"
              >
                <div className="w-8 h-8 rounded-full flex-shrink-0 bg-white/10 flex items-center justify-center font-bold text-sm text-white/80 group-hover:bg-orange-400 group-hover:text-black transition-colors">
                  {index + 1}
                </div>
                <div className="flex flex-1 items-center gap-4 overflow-hidden">
                  <h3 className="text-base font-bold whitespace-nowrap text-white/90 group-hover:text-orange-400 flex-shrink-0">
                    {facility.name}
                  </h3>
