...
interface PharmaciesPageProps {
  onBack: () => void;
  onNavigate?: (page: 'home' | 'party' | 'fashion' | 'food' | 'pharmacy') => void;
}

export const PharmaciesPage: React.FC<PharmaciesPageProps> = ({ onBack, onNavigate }) => {
  useEffect(() => {
    window.scrollTo(0, 0);
  }, []);
  ...
  const regions = Object.keys(pharmaciesData).sort((a, b) => {
  ...
  const pharmacies = useMemo(() => {
    // @ts-ignore
    return (pharmaciesData[selectedRegion] || []).slice(0, 50); // Show top 50 for performance
  }, [selectedRegion]);
    ...
    <div className="min-h-screen bg-black text-white selection:bg-white selection:text-black pb-20">
      ...
      <nav className="fixed top-0 left-0 right-0 z-50 h-20 px-6 sm:px-8 flex items-center justify-between backdrop-blur-md bg-black/50 border-b border-white/10">
        ...
        <div className="flex items-center gap-3 overflow-x-auto">
          {onNavigate && (
            <>
              <button onClick={() => onNavigate('pharmacy')} className="px-5 py-2.5 bg-blue-500 hover:bg-blue-400 text-white rounded-[14px] text-sm font-bold tracking-wider backdrop-blur-md transition-all active:scale-95 shadow-sm">동물약국</button>
      ...
      <section className="pt-40 pb-12 px-6 max-w-7xl mx-auto flex flex-col items-center text-center">
        ...
        >
          우리아가와 언제 어디서든 안심하고 함께할 수 있도록, 전국 동물약국 데이터를 제공합니다.<br/>
      ...
      <section className="px-6 max-w-7xl mx-auto">
        <div className="flex flex-col gap-3 w-full max-w-4xl mx-auto">
          {pharmacies.map((pharmacy: any, index: number) => {
            ...
            const shortAddress = pharmacy.address ? pharmacy.address.split(' ').slice(0, 2).join(' ') : '';
            const mapQuery = encodeURIComponent(`${pharmacy.name} ${shortAddress}`);
            return (
              <motion.a
                ...
                <div className="flex flex-1 items-center gap-4 overflow-hidden">
                  ...
                  {pharmacy.phone && (
                    <>
                      ...
                      <p className="text-sm text-white/40 hidden md:block">{pharmacy.phone}</p>
                    </>
                  )}
                </div>
